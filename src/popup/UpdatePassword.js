...
function UpdatePassword({ onClickCancel, setSnackBar }) {
    ...
    const updatePassword = async () => {
        ...
        let result = await Fetch(host + '/api/reset-password', 'POST', formData);
    ...
    return (
        <Box sx={{ background: theme.palette.background.paper }} className="w-1/2 h-3/4 rounded-xl py-3 px-5 overflow-y-scroll none-view-scroll max-sm:w-5/6">
            <Box className="w-20 h-20 rounded-full mx-auto my-5 flex justify-center items-center" sx={{ background: "#E8EEFD" }}>
                <LockResetOutlinedIcon className='text-purple-500 !text-5xl' />
            </Box>
            ...
            <Box className="py-3 flex justify-between max-sm:flex-col" sx={{flexDirection: language === 'en' && 'row-reverse'}}>
                <Button onClick={updatePassword} variant='contained' className='!bg-purple-500 w-2/5 !py-3 flex max-sm:w-full !text-white'>
                    {
                        sendWait ?
                ...
                <Button onClick={() => { resetInputs(); onClickCancel(); }} variant='contained' className='w-2/5 !py-3 flex !text-gray-500 !font-bold max-sm:w-full max-sm:!mt-3' color='inherit'><FormattedMessage id='cancel' /></Button>
            </Box>
        </Box>
    );
}
